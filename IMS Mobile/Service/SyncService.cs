    ...
    public class SyncService
    {
        ...
        public async Task SyncToSupabase()
        {
            try
            {
                ...
                await SyncUserPreferencesToSupabase();
        ...
        public async Task SyncFromSupabase()
        {
            try
            {
                ...
                await SyncUserPreferencesFromSupabase();
        ...
        public async Task ClearLocalData()
        {
            try
            {
                ...
                var userPreferences = App.UserPreferencesRepository?.GetItems().FirstOrDefault();
                if (userPreferences != null)
                {
                    userPreferences.Theme = Theme.Light;
                    userPreferences.Currency = Currency.USD;
                    userPreferences.DisplayName = "";
                    userPreferences.Avatar = "avatar1.png";
                    userPreferences.UserEmail = "";
                    App.UserPreferencesRepository?.SaveUserPreferences(userPreferences);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task ManualSyncToSupabase()
        {
            try
            {
                ...
                MirrorUserPreferencesToSupabase(),
        ...
        private async Task SyncUserPreferencesToSupabase()
        {
            if (_currentUserId == Guid.Empty) return;
            if (App.UserPreferencesRepository == null) return;
            var preferences = App.UserPreferencesRepository.GetUserPreferences();
            try
            {
                var existing = await _supabaseClient
                    ...
                    .Get();
                var preferencesDto = UserPreferencesDto.FromModel(preferences, _currentUserId.ToString());
                if (existing.Models.Count > 0)
                {
                    preferencesDto.Id = existing.Models[0].Id;
                    await _supabaseClient.From<UserPreferencesDto>().Where(x => x.Id == preferencesDto.Id).Update(preferencesDto);
                }
                else
                {
                    await _supabaseClient.From<UserPreferencesDto>().Insert(preferencesDto);
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Error syncing user preferences: {ex.Message}");
            }
        }
        ...
        private async Task SyncUserPreferencesFromSupabase()
        {
            if (_currentUserId == Guid.Empty) return;
            if (App.UserPreferencesRepository == null) return;
            try
            {
                ...
                if (existing.Models.Count > 0)
                {
                    var preferencesDto = existing.Models[0];
                    var preferences = preferencesDto.ToModel();
                    App.UserPreferencesRepository.SaveUserPreferences(preferences);
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Error syncing user preferences from Supabase: {ex.Message}");
            }
        }
            ...
            try
            {
                ...
                foreach (var transactionDto in supabaseTransactions.Models)
                {
                    try
                    {
                        ...
                        {
                            transaction.Products = relatedItems;
                        }
        ...
        private async Task MirrorUserPreferencesToSupabase()
        {
            if (_currentUserId == Guid.Empty) return;
            if (App.UserPreferencesRepository == null) return;
            var localPreferences = App.UserPreferencesRepository.GetUserPreferences();
            try
            {
                var existing = await _supabaseClient
                    .From<UserPreferencesDto>()
                    .Where(x => x.UserId == _currentUserId)
                    .Get();
                var preferencesDto = UserPreferencesDto.FromModel(localPreferences, _currentUserId.ToString());
                if (existing.Models.Count > 0)
                {
                    preferencesDto.Id = existing.Models[0].Id;
                    await _supabaseClient.From<UserPreferencesDto>().Where(x => x.Id == preferencesDto.Id).Update(preferencesDto);
            ...
            {
                Debug.WriteLine($"Error mirroring user preferences: {ex.Message}");
            }
        }
