...
namespace IMS_Mobile.DB
{
    public class BaseRepository<T> : IBaseRepository<T> where T : Entity, new()
    {
        ...
        public void UpdateItemWithChildren(T item, bool recursive = false, params IEnumerable[]? childCollections)
        {
            try
            {
                _connection.UpdateWithChildren(item);

                if (recursive && childCollections is not null)
                {
        ...
        //use cass : UpdateItemWithChildren(order, true, order.Lines);
        //UpdateItemWithChildren(order, true, order.Lines);
        //UpdateItemWithChildren(invoice, true, invoice.Items, invoice.Payments);
